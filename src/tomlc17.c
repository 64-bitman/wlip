...
static int
tab_add(
    toml_datum_t *tab, span_t newkey, toml_datum_t newvalue, const char **reason
)
{
    assert(tab->type == TOML_TABLE);
...
static int
datum_copy(
    toml_datum_t *dst, toml_datum_t src, pool_t *pool, const char **reason
)
{
...
toml_parse_file(FILE *fp)
{
    ...
    while (!feof(fp))
    {
        ...
        if (top == max)
        {
            ...
            {
                snprintf(
                    result.errmsg, sizeof(result.errmsg), "file is too big"
                );
                FREE(buf);
                return result;
            }
...
toml_parse(const char *src, int len)
{
    ...
    {
        snprintf(
            result.errmsg, sizeof(result.errmsg), "src[] must be NUL terminated"
        );
        goto bail;
    }
...
parse_key(parser_t *pp, token_t tok, keypart_t *ret_keypart)
{
    ...
    while (1)
    {
        ...
        {
            return SETERROR(
                pp->ebuf, tok.lineno, "expects a string in dotted-key"
            );
        }
    ...
    *ret_datum = mkdatum(TOML_ARRAY);
    ...
    for (;;)
    {
        ...
        if (!pelem)
        {
            datum_free(&value);
            return SETERROR(
                pp->ebuf, tok.lineno, "while parsing array: %s", reason
            );
        }
        *pelem = value;
...
parse_inline_table(parser_t *pp, token_t tok, toml_datum_t *ret_datum)
{
    ...
    *ret_datum = mkdatum(TOML_TABLE);
    ...
    for (;;)
    {
        ...
        {
            return SETERROR(
                pp->ebuf, tok.lineno, "inline table cannot be extended"
            );
        }
...
parse_std_table_expr(parser_t *pp, token_t tok)
{
    ...
    if (j < 0)
    {
        ...
        {
            return SETERROR(
                pp->ebuf, keylineno, "inline table cannot be extended"
            );
        }
        const char  *reason;
        toml_datum_t newtab = mkdatum(TOML_TABLE);
...
parse_array_table_expr(parser_t *pp, token_t tok)
{
    ...
    for (int i = 0; i < keypart.nspan; i++)
    {
        ...
        if (value->type == TOML_ARRAY)
        {
            ...
            {
                return SETERROR(
                    pp->ebuf, keylineno, "cannot expand array %s", curkey.ptr
                );
            }
            if (value->u.arr.size <= 0)
            {
                return SETERROR(
                    pp->ebuf, keylineno, "array %s has no elements", curkey.ptr
                );
            }
            value = &value->u.arr.elem[value->u.arr.size - 1];
            if (value->type != TOML_TABLE)
        ...
        // keypart not found
        return SETERROR(
            pp->ebuf, keylineno, "cannot locate table at key %s", curkey.ptr
        );
    }
...
parse_norm(parser_t *pp, token_t tok, span_t *ret_span)
{
    ...
    while (*p)
    {
        ...
        case 'x':
        {
            ...
            {
                return SETERROR(
                    pp->ebuf, tok.lineno, "error converting UCS %s to UTF8", buf
                );
            }
            dst += n;
            p += 2 + 2; // \xNN
        ...
        case 'U':
        {
            ...
            if (0xD800 <= ucs && ucs <= 0xDFFF)
            {
                ...
                // point)
                return SETERROR(
                    pp->ebuf, tok.lineno, "invalid UTF8 char \\u%04x", ucs
                );
            }
            int n = ucs_to_utf8(ucs, dst);
            if (n < 0)
        ...
        default:
            return SETERROR(
                pp->ebuf, tok.lineno, "internal: unknown escape char \\%c", p[1]
            );
        }
    }
    *dst = 0;
    ...
    while (1)
    {
        ...
        if (ch == ' ' || ch == '\t')
        {
            ...
            if (ch != '\n')
            {
                ...
                // before newline
                return SETERROR(
                    sp->ebuf, sp->lineno, "bad escape char in string"
                );
            }
            // fallthru
        }
...
scan_multiline_litstring(scanner_t *sp, token_t *tok)
{
    ...
    *tok = mktoken(sp, TOK_MLLITSTRING);
    while (1)
    {
        ...
        {
            return SETERROR(
                sp->ebuf, sp->lineno, "unterminated multiline lit string"
            );
        }
        if (!(is_valid_char(ch) || (ch && strchr(" \t\n", ch))))
        {
    ...
    *tok = mktoken(sp, TOK_FIN);
    ...
    case '#':
        ...
        while (!S_MATCH('\n'))
        {
            ...
            {
                return SETERROR(
                    sp->ebuf, sp->lineno, "bad control char in comment"
                );
            }
        }
